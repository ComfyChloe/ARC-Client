...
interface DirectionalParameterNames {
...
interface LeashConfig {
  ...
  DirectionalParameters: DirectionalParameterNames
}

interface OscMessageData {
  address: string
  value: number
}
...
class OSCLeashConfig {
  ...
  Leashes!: string[]
  DirectionalParameters!: DirectionalParameterNames
  constructor(configData: LeashConfig | null = null) {
    this.setSettings(configData || DefaultConfig)
  }

  setSettings(configJson: LeashConfig): void {
    try {
      ...
      debug.logError(`OSCLeash: Malformed config, using defaults: ${(error as Error).message}`)
...
class Leash {
  ...
  Posed: boolean
  Active: boolean
  Z_Positive_ParamName: string
  Z_Negative_ParamName: string
  X_Positive_ParamName: string
  X_Negative_ParamName: string
  Y_Positive_ParamName: string
  Y_Negative_ParamName: string
  constructor(paraName: string, contacts: DirectionalParameterNames, settings: OSCLeashConfig) {
    this.Name = paraName
...
class OSCPackageController {
  leashes: Leash[]
  oscQuery: EventEmitter | null
  oscService: OscService | null
  addonInstance: OSCLeashAddon | null
  listeners: Map<string, (oscData: OscMessageData) => void>
  onLeashActivate: ((leash: Leash) => void) | null
  onLeashDeactivate: ((leash: Leash) => void) | null
  constructor(leashCollection: Leash[], oscQuery: EventEmitter | null, oscService: OscService | null, addonInstance: OSCLeashAddon | null = null) {
    ...
    this.listeners = new Map<string, (oscData: OscMessageData) => void>()
    this.onLeashActivate = null
    this.onLeashDeactivate = null
  }
  listen(): void {
  ...
  listenLeash(leashCollection: Leash[]): void {
  ...
  registerListener(address: string, callback: (value: number) => void): void {
    if (this.oscQuery) {
      // Create a wrapper callback that filters OSC-Query messages by address
      const wrappedCallback = (oscData: OscMessageData) => {
        try {
          if (oscData.address === address) {
            callback(oscData.value)
          }
        } catch (error) {
          debug.error(`Error in OSCLeash listener for ${address}: ${(error as Error).message}`, {
            address,
            value: oscData?.value,
  ...
  updateGrabbed(currLeash: Leash, value: unknown): void {
...
class OSCLeashProgram {
  ...
  constructor(oscQuery: EventEmitter | null, oscService: OscService | null, addonInstance: OSCLeashAddon | null = null) {
    this.oscQuery = oscQuery
    this.oscService = oscService
    this.addonInstance = addonInstance
    this.running = false
    this.activeIntervals = new Map<string, ReturnType<typeof setInterval>>() // Track active polling intervals
  }
  /**
  ...
  updateProgram(runBool: boolean): void {
    this.running = runBool
  }
  // Start active polling when leash is grabbed
  startLeashMonitoring(leash: Leash): void {
    ...
    const intervalId = setInterval(() => {
      try {
        ...
        debug.error(`Error in leash monitoring loop for ${leash.Name}: ${(error as Error).message}`, {
  ...
  stopLeashMonitoring(leash: Leash): void {
  ...
  processLeashMovement(leash: Leash): void {
    ...
    this.notifyMovementUpdate(leash, VerticalOutput, HorizontalOutput, 0)
  }

  notifyMovementUpdate(leash: Leash, vertical: number, horizontal: number, run: number): void {
    ...
    if (typeof this.addonInstance?.onMovementUpdate === 'function') {
      try {
        ...
        debug.error(`Error in movement update callback: ${(error as Error).message}`, {
          leashName: leash.Name,
          stack: (error as Error).stack
        })
      }
  ...
  async leashRun(leash: Leash): Promise<void> {
    if (!leash.Active) {
      return
    }
  ...
  leashOutput(vert: number, hori: number, runType: number, settings: OSCLeashConfig): void {
    ...
    try {
      ...
      debug.error(`Error sending leash output OSC messages: ${(error as Error).message}`, {
        vertical: vert,
        horizontal: hori,
        runType,
        stack: (error as Error).stack
...
class OSCLeashAddon {
  ...
  settings: OSCLeashConfig
  leashes: Leash[]
  discoveredLeashes: Set<string>
  packageController: OSCPackageController | null
  ...
  constructor() {
    ...
    this.leashes = []
    this.discoveredLeashes = new Set<string>() // Track which leashes have been detected
   ...
   */
  setMovementCallback(callback: (data: unknown) => void): void {
  ...
  loadConfig(): LeashConfig {
    try {
      ...
      debug.logError(`Failed to load OSCLeash config: ${(error as Error).message}`)
    }
  ...
  saveConfig(): boolean {
    try {
      ...
      configManager.updateOSCLeashConfig(config as unknown as Record<string, unknown>)
      debug.info('OSCLeash config saved via config manager')
      return true
    } catch (error) {
      debug.logError(`Failed to save OSCLeash config: ${(error as Error).message}`)
      return false
    }
  }
  ...
  start(oscQuery: EventEmitter | null = null, oscService: OscService | null = null): boolean {
    ...
    try {
      ...
      debug.logError(`Failed to start OSCLeash: ${(error as Error).message}`)
      this.enabled = false
  ...
  stop(): void {
    ...
    try {
      ...
      debug.warn(`OSCLeash: Could not send stop signals during shutdown: ${(error as Error).message}`)
    }
  ...
  updateConfig(newConfig: LeashConfig): boolean {
    try {
      ...
      debug.logError(`Failed to update OSCLeash config: ${(error as Error).message}`)
      return false
    }
  }
